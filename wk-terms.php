...
*/
    ...
    if ($r['hierarchical']) {
      ...
      $out = $this->level($terms, 0, $r);
    if($r['h5']){
        $out = "<div class='WK_terms'><h5>{$r['h5']}</h5>\r\n$out</div>\r\n";
    }if($r['h4']){
        $out = "<div class='WK_terms'><h4>{$r['h4']}</h4>\r\n$out</div>\r\n";
    }if($r['h3']){
        $out = "<div class='WK_terms'><h3>{$r['h3']}</h3>\r\n$out</div>\r\n";
    }if($r['h2']){
        $out = "<div class='WK_terms'><h2>{$r['h2']}</h2>\r\n$out</div>\r\n";
    }if($r['h1']){
        $out = "<div class='WK_terms'><h1>{$r['h1']}</h1>\r\n$out</div>\r\n";
    }
    return $out;
  }
